...
maximize TotalProfit: sum{i in FURNITURE,j in FINISH} SalesPrice[i,j] * Produce[i,j] 
					-  sum{i in FURNITURE, j in FINISH} LumberRequired[i] * LumberCost * Produce[i,j];
...
subject to LumberLimit: sum{i in FURNITURE, j in FINISH} LumberRequired[i] * Produce[i,j] <= LumberAvailable;

# Limit on available labor (in manhours)
subject to LaborLimit: sum{i in FURNITURE, j in FINISH} LaborRequired[i,j] * Produce[i,j] <=  LaborAvailable;
